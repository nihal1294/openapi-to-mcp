...
def _run_detector(
    ...
    monkeypatch.setenv("BEFORE_SHA", "old-sha")
...
def test_changed_version_without_tag_requests_release(
    ...
    assert "release_reason=version-bump" in contents
...
def test_unchanged_version_with_existing_tag_skips_release(
    ...
    assert "release_reason=none" in contents


def test_missing_tag_requests_release_for_unchanged_version(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    output = _run_detector(monkeypatch, tmp_path, "0.9.0", "")
    contents = output.read_text(encoding="utf-8")
    assert "should_release=true" in contents
    assert "release_reason=missing-tag" in contents

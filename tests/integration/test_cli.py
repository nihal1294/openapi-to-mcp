...
def test_generate_swagger_operation_scheme_override_fails_or_skips(
    runner: CliRunner, tmp_path: Path
) -> None:
    spec_path = tmp_path / "swagger-schemes.json"
    spec_path.write_text(
        json.dumps(
            {
                "swagger": "2.0",
    ...
    strict_output = tmp_path / "strict-schemes"
    skipped_output = tmp_path / "skipped-schemes"
    ...
    report = json.loads((skipped_output / "generation_report.json").read_text())
    assert report["mapped_tools"] == 1
    assert report["skipped_operations"][0]["path"] == "/secure"
